...
#   users = User.objects.filter(Q(type='测试') | Q(type='管理员')).values_list('name', flat=True)
    users = User.objects.filter(Q(type='测试')& ~Q(name='鲁圣')& ~Q(name='马燕')& ~Q(name='唐浩明')).values_list('name', flat=True)
    users_test = User.objects.filter(Q(type='测试')& ~Q(name='鲁圣')& ~Q(name='马燕')& ~Q(name='唐浩明')).values_list('name', flat=True)
    for user in users_test:
        work_dict[user]=0
        user_tasks_finished = Taskdetail.objects.filter(Q(name=user) & ~Q(canbegan__contains='完成')& Q(begantime__lte=today)& Q(endtime__gte=today))
...
#   users = User.objects.filter(type='测试').values_list('name', flat=True)
    users = User.objects.filter(Q(type='测试')& ~Q(name='鲁圣')& ~Q(name='马燕')& ~Q(name='唐浩明')).values_list('name', flat=True)
    tasks_unfinished_num = Taskdetail.objects.filter(Q(name=username) & ~Q(canbegan__contains='完成')& Q(begantime__lte=today)& Q(endtime__gte=today)).count()
    context = {
        'tasks':tasks,
        'users':users,
        'username':username,
